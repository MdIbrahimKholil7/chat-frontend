...
const MessengerHeadActiveUser = ({activeUsers}:Props) => {
    ...
    console.log(friend)
    const isActive=activeUsers?.some((u:SocketUser)=>u?.userId === friend?.friend?._id)
  return (
    <div>
      <div className="relative w-[50px] h-[50px] cursor-pointer">
        ...
        />
        <div className={`w-[10px] h-[10px] ${isActive ? 'bg-green-500':'bg-slate-400'} rounded-full absolute top-[3px] right-[3px]`}></div>
      </div>
    </div>
  );
};
